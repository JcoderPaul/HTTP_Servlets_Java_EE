...
- **paramValues** - Список значений параметров, которые были переданы серверу приложений. Формат отличается от предыдущего контекста. Если там param имел тип HashMap<String, String>, то здесь HashMap<String, String [] >.
...
- **initParam** - Конфигурационные параметры, указанные для страницы или сервлета в дескрипторе приложений web.xml
...
Чтобы было вычислено выражение атрибута value его необходимо оформить как выражение EL (Expression Language) в виде "${16+64*2}" .
...
Тег `c:set` используется для определения переменной подобно `<jsp:setProperty>`. 
...
и массивам. Для этого используется параметр target. 
...
JSTL пример с использованием тега `<c:forEach>` и компонента JavaBean Создадим тестовый компонент 
GoodsBean, к которому будем обращаться в цикле `<c:forEach>`:
...
Для обработки исключений в теле JSP следует использовать тэг `<c:catch>`. Пример использования `<c:catch>`:
...
---
**Доп. материал:**
- [The Java EE 5 Tutorial (by ORACLE)](https://docs.oracle.com/javaee/5/tutorial/doc/bnake.html)
- [A Guide to the JSTL Library](https://www.baeldung.com/jstl)
- [JSTL Tutorials - Complete JSTL Tag References](https://www.codejava.net/jstl-tutorials)
...
- [How to set JSTL locale from Java code?](https://stackoverflow.com/questions/6126542/how-to-set-jstl-locale-from-java-code)
- [EE JSP: Generating Dynamic Content with JSTL Tag Libraries](https://www.javacodegeeks.com/2015/02/ee-jsp-generating-dynamic-content-jstl-tag-libraries.html)
- [Introduction to the Java Standard Tag Library](https://www.sitepoint.com/java-standard-tag-library/)
